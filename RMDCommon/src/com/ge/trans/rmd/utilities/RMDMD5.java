...
public final class RMDMD5 {
    ...
    public void update(byte[] newbuf, int bufstart, int buflen) {
        ...
        if (temp != 0) {
            ...
            temp = 64 - temp;
            if (len < temp) {
                System.arraycopy(newbuf, localbufstart, in, position, len);
                return;
            }
            System.arraycopy(newbuf, localbufstart, in, position, temp);
            transform();
            localbufstart += temp;
            len -= temp;
        }
        ...
        while (len >= 64) {
            System.arraycopy(newbuf, localbufstart, in, 0, 64);
            transform();
            localbufstart += 64;
            len -= 64;
        }
        ...
        System.arraycopy(newbuf, localbufstart, in, 0, len);
    }
    ...
    public static String encryptMD5(String text) {
        ...
        String passwrd = text;
        int len = text.length() + (text.length() / 2);
     ...
     * @Description:
     */
    private void zeroByteArray(byte[] byteArray) {
     ...
     * @Description:
     */
    private void zeroByteArray(byte[] byteArray, int start, int length) {
     ...
     * @Description:
     */
    private void setByteArray(byte[] byteArray, byte val, int start, int length) {
        int i;
     ...
     * @Description:
     */
    private void zeroIntArray(int[] intArray, int start, int length) {
     ...
     * @Description:
     */
    private int stepMD5(Fcore f, int w, int x, int y, int z, int data, int s) {
        int localX = w;
        localX += f.f(x, y, z) + data;
        localX = localX << s | localX >>> (32 - s);
        localX += x;
        return localX;
    }
     ...
     * @Description:
     */
    private int get32BitLSBFirst(byte[] byteArray, int offSet) {
     ...
     * @Description:
     */
    private void put32BitLsbFirst(byte[] byteArray, int offSet, int value) {
