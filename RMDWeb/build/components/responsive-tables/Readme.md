...
## Getting Started
`responsive-tables` works by searching for the classes `essential` and `optional` on a table's `th` elements, if that table has the class `responsive`.  JavaScript then adds these classes to subsequent `td`s with the same row positions.
...
- Add the class `responsive` to each `table` you want to have this treatment.
